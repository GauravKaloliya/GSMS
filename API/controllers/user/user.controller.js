...
// Helper to expire current valid record
const expireCurrentValidRecord = async (client, table, user_id) => {
  await client.query(`UPDATE ${table} SET valid_to = now() WHERE user_id = $1 AND valid_to IS NULL`, [user_id]);
}
...
router.post('/email', verifyToken, async (req, res) => {
  ...
  // Optional: validate email format here with validator package
  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT pgp_sym_decrypt(email, current_setting('pg.encrypt_key')) AS email
       FROM user_email WHERE user_id = $1 AND valid_to IS NULL`,
      [uid]
    );
